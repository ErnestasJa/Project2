...
namespace core {
AxisAlignedBoundingBox::AxisAlignedBoundingBox() : m_halfSize(0.5) {
  // ctor
}

AxisAlignedBoundingBox::AxisAlignedBoundingBox(const glm::vec3 &center, const glm::vec3 &halfSize)
...
void AxisAlignedBoundingBox::Reset(const glm::vec3 &point) {
  m_center = point;
  m_halfSize = glm::vec3(0);
}

void AxisAlignedBoundingBox::AddPoint(const glm::vec3 &point) {
...
void AxisAlignedBoundingBox::Translate(const glm::vec3 &point) { m_center += point; }

bool AxisAlignedBoundingBox::ContainsPoint(const glm::vec3 &point) const {
...
bool AxisAlignedBoundingBox::IntersectsWith(const AxisAlignedBoundingBox &other) const {
  return (glm::abs(m_center.x - other.GetCenter().x) <=
          m_halfSize.x + other.GetHalfSize().x) &&
         (glm::abs(m_center.y - other.GetCenter().y) <=
          m_halfSize.y + other.GetHalfSize().y) &&
         (glm::abs(m_center.z - other.GetCenter().z) <=
          m_halfSize.z + other.GetHalfSize().z);
}

float AxisAlignedBoundingBox::SweepCollidesWith(const AxisAlignedBoundingBox &other, const glm::vec3 &vel,
...
bool AxisAlignedBoundingBox::IntersectsWith(const glm::vec3 &center,
...
bool AxisAlignedBoundingBox::CollidesWithRay(const glm::vec3 &rayStart,
                           const glm::vec3 &rayDirectionInverse) const {
  double tx1 =
      ((m_center.x - m_halfSize.x) - rayStart.x) * rayDirectionInverse.x;
  double tx2 =
      ((m_center.x + m_halfSize.x) - rayStart.x) * rayDirectionInverse.x;
  ...
  double ty1 =
      ((m_center.y - m_halfSize.y) - rayStart.y) * rayDirectionInverse.y;
  double ty2 =
      ((m_center.y + m_halfSize.y) - rayStart.y) * rayDirectionInverse.y;
  ...
  double tz1 =
      ((m_center.z - m_halfSize.z) - rayStart.z) * rayDirectionInverse.z;
  double tz2 =
      ((m_center.z + m_halfSize.z) - rayStart.z) * rayDirectionInverse.z;
...
glm::vec3 AxisAlignedBoundingBox::GetHalfSize() const { return m_halfSize; }

glm::vec3 AxisAlignedBoundingBox::GetCenter() const { return m_center; }

glm::vec3 AxisAlignedBoundingBox::GetMin() const { return m_center - m_halfSize; }

glm::vec3 AxisAlignedBoundingBox::GetMax() const { return m_center + m_halfSize; }

void AxisAlignedBoundingBox::SetCenter(const glm::vec3 &point) { m_center = point; }

std::vector<glm::vec3> AxisAlignedBoundingBox::CalculatePoints() const {
